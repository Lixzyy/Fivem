...
function RageUI.PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (Index == nil or (CurrentMenu.Index == Index)) then
            ...
            -- background (black) instead of gradient sprite
            RenderRectangle(CurrentMenu.X, CurrentMenu.Y + Percentage.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, Percentage.Background.Width + CurrentMenu.WidthOffset, Percentage.Background.Height, 0, 0, 0, 255)
